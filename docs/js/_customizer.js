jQuery(function($) {
	var $rtl, $style, $customizer = $("#customizer"),
		...
		coptions = {
			styles: [{
				name: "Default",
				url: ["../themes/default/clique-customizer.less"],
				config: "../themes/default/customizer.json"
			}, {
				name: "Gradient",
				url: ["../themes/gradient/clique-customizer.less"],
				config: "../themes/gradient/customizer.json"
			}, {
				name: "Almost Flat",
				url: ["../themes/almost-flat/clique-customizer.less"],
				config: "../themes/almost-flat/customizer.json"
			}]
		};
	...
	$error.on({
		show: function(e, error) {
			$error.html($.mustache('<h1 class="h3">Less {{type}} Error</h1><p>{{message}}</p>', error)).show();
			$iframe.css("opacity", "0");
		},
		hide: function() {
			$error.hide();
	...
	$("a[download='clique.css']").on("click", function(e) {
			...
			if(f && f.name) {
				...
				r.onload = function(e) {
					...
					var contents = e.target.result,
						lessvar, vars = {},
						$options = $customizer.data("customizer").$options,
						theme;
	...
	function renderPreview(style) {
		...
		$.less.getCSS(style.less, {
			...
			$iframe[0].contentWindow.jQuery.Clique.langdirection = $rtl ? "right" : "left";
	...
	function downloadCSS(a, style) {
		...
		$.less.getCSS(style.less, $.extend(options, {
			...
			if($download) {
				a.attr("href", $url.createObjectURL(new Blob([css], {
